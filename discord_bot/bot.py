...
def init_db():
    ...
    with closing(connect_db()) as db:
        with open(f"{path}/database/schema.sql", "r") as f:
...
def connect_db():
    path = os.path.dirname(os.path.abspath(__file__)).replace('\\', '/')
    return sqlite3.connect(f"{path}/database/database.db")
        ...
        if file.endswith(".py"):
            ...
            try:
                try:
                    ...
                    await bot.load_extension(f"{path}/cogs/{file}.{extension}")
                    print(f"Loaded extension '{extension}'")
...
class bot_class(threading.Thread):
