...
def get_status(addr, port=25565):
    ...
    return json.loads(read(sock, length)), ping
...
class Scanner:
    ...
    def update_ping(self, ip_id, ping):
        if ping:
            self.db.execute(f'UPDATE ip SET ping = {str(ping)} WHERE nr = {str(ip_id)}')
        return f'Ping: {ping}'
    ...
    def update(self):
        for ip_data in self.data:
            ...
            data, ping = self.get_data(ip, port)
            ...
            if data != 'Offline' and data != 'Connection reset' and data != 'Connection refused' and data != 'Type error':
                ...
                print(self.update_ping(ip_id, ping))
